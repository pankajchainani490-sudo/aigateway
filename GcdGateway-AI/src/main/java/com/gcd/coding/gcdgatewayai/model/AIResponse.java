 ...
 *
 * 遵循OpenAI Chat Completions API响应格式
 * 包含响应ID、模型信息、生成内容、使用量等信息
 *
...
public class AIResponse {
    ...
    /**
     * 实际处理的模型名称
     * 可能与请求中的model不同（如请求"deepseek-chat"，返回"deepseek-v4-flash"）
     */
     ...
     *
     * 流式响应时使用delta字段，非流式使用message字段
     */
    private List<Choice> choices = new ArrayList<>();

    /**
     * Token使用量统计
     * 包含输入token数、输出token数、总token数
     * 用于计费和监控
     */
    private Usage usage;
     ...
     * 响应选项 - 表示AI生成的一个完整回复
     *
     * 在非流式响应中，message字段包含完整的AI回复
     * 在流式响应中，delta字段包含增量内容（用于SSE传输）
     */
    @Data
